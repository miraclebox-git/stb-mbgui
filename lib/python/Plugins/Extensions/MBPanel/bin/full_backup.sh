...
if [ -f /proc/stb/info/boxtype ] ; then
	...
	EXTRA=$DIRECTORY/Miraculous_fullbackup_$MODEL/$DATE
else
	echo "No supported receiver found!"
	exit 0
fi
...
if [ $TYPE = "miraclebox" ] ; then
	...
	cd $EXTRA
	zip $DIRECTORY/Miraculous_fullbackup_$MODEL/$DATE/egami-$MODEL-image-$DATE-usb.zip $TYPE/$MODEL/*
else
	mkdir -p $EXTRA/$MODEL
	mv $WORKDIR/root.ubifs $MAINDEST/rootfs.bin
	mv $WORKDIR/vmlinux.gz $MAINDEST/kernel.bin
	...
	cd $EXTRA
	zip $DIRECTORY/Miraculous_fullbackup_$MODEL/$DATE/egami-$MODEL-image-$DATE-usb.zip $MODEL/*
fi
