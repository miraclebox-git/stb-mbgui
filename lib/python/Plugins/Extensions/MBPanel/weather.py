...
class MBMeteoMain(Screen):
    skin = '''<screen position="center,center" size="1280,720" title="Miraclebox Weather" flags="wfNoBorder">
	  <ePixmap position="0,0" size="1280,720" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/MBPanel/icons/weather/100.png" />
    ...
    def updateInfo(self):
        ...
        try:
            ...
            if dom:
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/MBPanel/icons/weather/%s.png' % str(weather_data['condition']['code'])
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/MBPanel/icons/weather/small/%s.png' % str(weather_data['forecasts'][0]['code'])
                myicon = self.checkIcon(icon)
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/MBPanel/icons/weather/small/%s.png' % str(weather_data['forecasts'][1]['code'])
                myicon = self.checkIcon(icon)
                png = loadPic(myicon, 100, 100, 0, 0, 0, 0)
                ...
                myicon = '/usr/lib/enigma2/python/Plugins/Extensions/MBPanel/icons/weather/red.png'
    ...
    def checkIcon(self, localfile):
        if fileExists(localfile):
            ...
            url = localfile.replace('/usr/lib/enigma2/python/Plugins/Extensions/MBPanel/icons', 'http://www.vuplus-community.net/bhaddons')
            handler = urlopen(url)
            if handler:
    ...
    def key_red(self):
        ...
        self.session.openWithCallback(self.goSelect, InputBox, title=msg, windowTitle=_('Change city'), text='Stockholm')

    def goSelect(self, city):
        if city is not None:
            self.session.openWithCallback(self.updateInfo, MBMeteoSelectCity, city)


class MBMeteoSelectCity(Screen):
